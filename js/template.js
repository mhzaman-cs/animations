...
let _codepenIDRegex = /codepen\.io\/[^/]+\/(?:pen|debug|fullpage|fullembedgrid)\/([^?#]+)/; // Why not?
...
let ctx = canvas.getContext('2d', {
  desynchronized: window.canvasOptions && window.canvasOptions.desynchronized !== undefined ? window.canvasOptions.desynchronized : _defaulCanvasOptions.desynchronized // preserveDrawingBuffer: true // WebGL
...
let mouseUpdate = -Infinity,
    ...
    mousePos = null,
    mousePosPrev = null,
    mouseDownTime = -Infinity,
    mouseDownPos = null,
    mouseUpTime = -Infinity,
    mouseUpPos = null,
    mouseEnterTime = -Infinity,
    mouseExitTime = -Infinity;

function updateMouse(e, eventName) {
  // Modified from p5.js
  if (_logMouseEvents) {
...
canvas.addEventListener('mouseenter', e => {
  ...
  mouseIn = true;
  mouseEnterTime = e.timeStamp; // mouseExitTime = -Infinity;
});
canvas.addEventListener('mouseleave', e => {
  updateMouse(e, 'mouseleave');
  mouseIn = mouseDown = false; // mouseEnterTime = -Infinity;
...
window.addEventListener('load', () => {
  ...
  Object.assign(_canvasOptions, _defaulCanvasOptions, 'canvasOptions' in window ? window.canvasOptions : {});
...
function clear(x, y, w, h) {
  ...
  } else if (_canvasOptions.centered && _canvasCurrentlyCentered
  /*  && x !== null */
  ) {
...
function point(x = 0, y = 0, r = 0, g = 0, b = 0, a = 255, doPut_ = true) {// let imgData = ctx.createImageData(1, 1);
  // imgData.data[0] = r;
  // imgData.data[1] = g;
  // imgData.data[2] = b;
  // imgData.data[3] = a;
...
function xyToI(x, y, w, h) {
  ...
  if (w === undefined) w = 1; // if(h === undefined) h = Infinity;
...
class Vector {
  ...
  mag() {
    return Math.sqrt(this.magSq()); // return hypot(this.x, this.y);
  }

  mag3D() {
    return Math.sqrt(this.magSq3D()); // return hypot(this.x, this.y);
  }
...
function getTimeArray(timestamp = null) {
  ...
  let arr = [timestamp.getHours(), timestamp.getMinutes(), timestamp.getSeconds(), timestamp.getMilliseconds()];
  return arr;
}
